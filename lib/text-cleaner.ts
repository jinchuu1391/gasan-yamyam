...
async function aiCleanText(text: string): Promise<{
  ...
  try {
    // Gemini 2.0 Flash 모델 사용 (최신 성능, 빠른 처리)
    const modelsToTry = [
      'gemini-2.0-flash',     // 최신 2.0 모델
      'gemini-1.5-flash',     // 1.5 Flash 백업
      'gemini-pro',           // 기본 모델 백업
      'models/gemini-pro'     // 풀 경로 백업
    ];
...
응답:`;
    ...
    if (cleanedText.includes('메뉴 정보 없음') || 
...
): Promise<{ id: string; result: Awaited<ReturnType<typeof cleanMenuText>> }[]> {
  console.log(`🧹 Starting batch text cleaning for ${textList.length} items...`);
